...
"""
Component CylinderCollisionModel

.. autofunction:: CylinderCollisionModel
...
def CylinderCollisionModel(attachedTo , name='CylinderCollisionModel', printLog=0, tags=[], bbox=array([[ 1.79769313e+308,  1.79769313e+308,  1.79769313e+308],
       [-1.79769313e+308, -1.79769313e+308, -1.79769313e+308]]), componentState='Undefined', listening=0, active=1, moving=1, simulated=1, selfCollision=0, proximity=0.0, contactStiffness=10.0, contactFriction=0.0, contactRestitution=0.0, contactResponse='', color=array([1., 0., 0., 1.], dtype=float32), group=[], numberOfContacts=0, radii=array([], dtype=float64), heights=array([], dtype=float64), defaultRadius=0.5, defaultHeight=2.0, defaultLocalAxis=array([0., 1., 0.]), **kwargs):
...
Note that this is only indicative, and in particular if both collision models specify a different class it is up to the manager to choose.
		 ...
		 radii: Radius of each cylinder

		 heights: The cylinder heights

		 defaultRadius: The default radius

		 defaultHeight: The default height

		 defaultLocalAxis: The default local axis cylinder is modeled around


    """
    return attachedTo.createObject("CylinderCollisionModel" , name=name, printLog=printLog, tags=tags, bbox=bbox, componentState=componentState, listening=listening, active=active, moving=moving, simulated=simulated, selfCollision=selfCollision, proximity=proximity, contactStiffness=contactStiffness, contactFriction=contactFriction, contactRestitution=contactRestitution, contactResponse=contactResponse, color=color, group=group, numberOfContacts=numberOfContacts, radii=radii, heights=heights, defaultRadius=defaultRadius, defaultHeight=defaultHeight, defaultLocalAxis=defaultLocalAxis, **kwargs)
