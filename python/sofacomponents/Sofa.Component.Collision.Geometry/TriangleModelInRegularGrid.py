...
"""
Component TriangleModelInRegularGrid

.. autofunction:: TriangleModelInRegularGrid
...
def TriangleModelInRegularGrid(attachedTo , name='TriangleModelInRegularGrid', printLog=0, tags=[], bbox=array([[ 1.79769313e+308,  1.79769313e+308,  1.79769313e+308],
       [-1.79769313e+308, -1.79769313e+308, -1.79769313e+308]]), componentState='Undefined', listening=0, active=1, moving=1, simulated=1, selfCollision=0, proximity=0.0, contactStiffness=10.0, contactFriction=0.0, contactRestitution=0.0, contactResponse='', color=array([1., 0., 0., 1.], dtype=float32), group=[], numberOfContacts=0, bothSide=0, computeNormals=1, useCurvature=0, **kwargs):
...
Note that this is only indicative, and in particular if both collision models specify a different class it is up to the manager to choose.
		 ...
		 bothSide: activate collision on both side of the triangle model

		 computeNormals: set to false to disable computation of triangles normal

		 useCurvature: use the curvature of the mesh to avoid some self-intersection test


    """
    return attachedTo.createObject("TriangleModelInRegularGrid" , name=name, printLog=printLog, tags=tags, bbox=bbox, componentState=componentState, listening=listening, active=active, moving=moving, simulated=simulated, selfCollision=selfCollision, proximity=proximity, contactStiffness=contactStiffness, contactFriction=contactFriction, contactRestitution=contactRestitution, contactResponse=contactResponse, color=color, group=group, numberOfContacts=numberOfContacts, bothSide=bothSide, computeNormals=computeNormals, useCurvature=useCurvature, **kwargs)
